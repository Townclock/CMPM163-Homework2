        ...
        float tx0y2 = tex2D( _MainTex, i.uv + texel * float2( 0,  -1 ) ).r ;
		  ...
		  float3 normal = float3(tx1y0 - tx2y0, 0.5, tx0y1 - tx0y2);
		    ...
		    //vReflect.x *= abs(_SinTime.y);
				...
				//sample the skybox
			float4 refractColor;
			if ((i.uv.x % 0.2 < 0.1) && ! (i.uv.y % 0.2 < 0.1) || !(i.uv.x % 0.2 < 0.1) &&  (i.uv.y % 0.2 < 0.1) )
				refractColor = texCUBE(_Cube, (vIncident));
			else
				refractColor = texCUBE(_Cube, (vIncident  + normal/2)/1.5);
